...
const CityForm = ({ isOpen, onClose, setEditData,isParentRender }) => {
    const [city, setCity] = useState({ name: '', status: '' });
    ...
    useEffect(() => {
      ...
      const handleSubmit = async (e) => {
        ...
        if (setEditData?._id) {
          try {
            const update = await put(CITY_END_POINT.update(setEditData._id), city);
          ...
          try {
            const response = await post(CITY_END_POINT.create(), city);
          ...
          <div className="relative bg-white p-8 rounded-lg  dark:border-strokedark dark:bg-boxdark w-full max-w-md max-h-full">
            ...
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                 {setEditData?._id?"Update City":"Create New City"} 
              </h3>
              <button
                  onClick={() => {
                    onClose();
